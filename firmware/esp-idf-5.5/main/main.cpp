...
static constexpr uint8_t TFT_ROT = 3;
TFT_eSPI tft;

// ========= PINS =========
static constexpr int PIN_NTC = 35; // ADC input-only
static constexpr int PIN_SSR = 27; // SSR/MOSFET output
...
// ========= XPT2046 RAW TOUCH =========
static constexpr uint8_t TOUCH_CMD_X = 0xD0;
static constexpr uint8_t TOUCH_CMD_Y = 0x90;
...
static constexpr float HYST = 2.0f;
...
// ========= Manual ramp + hold =========
static float    manualRampSetC   = 25.0f;
static uint32_t manualRampLastMs = 0;

static constexpr float MANUAL_RAMP_CPS    = 0.6f;
static constexpr float MANUAL_SYNC_BAND_C = 3.0f;

// ========= Burst PWM settings (saved) =========
static float    APPROACH_BAND_C = 25.0f;
static uint32_t WINDOW_MS       = 2000;

// Max power limit (saved)
static float MAX_DUTY = 0.40f;

// ========= Temp offset (saved) =========
static float TEMP_OFFSET_C = 0.0f;
...
static constexpr float PWR_STEP = 0.05f;
...
static inline bool touchPressedFast(){
...
// Raw touch -> screen coords for rotation 3
static bool getTouchXY(int &x, int &y){
  if(!touchPressedFast()) return false;

  uint16_t x1 = touchRead12(TOUCH_CMD_X);
  uint16_t y1 = touchRead12(TOUCH_CMD_Y);
  delayMicroseconds(150);

  if(!touchPressedFast()) return false;
  uint16_t x2 = touchRead12(TOUCH_CMD_X);
  uint16_t y2 = touchRead12(TOUCH_CMD_Y);
  delayMicroseconds(150);

  if(!touchPressedFast()) return false;
  uint16_t x3 = touchRead12(TOUCH_CMD_X);
  uint16_t y3 = touchRead12(TOUCH_CMD_Y);

  int rawX = median3((int)x1, (int)x2, (int)x3);
  int rawY = median3((int)y1, (int)y2, (int)y3);

  x = map(rawY, YMIN, YMAX, W - 1, 0);
  y = map(rawX, XMIN, XMAX, 0, H - 1);
...
// ========= CONTROL =========
static bool computeRelayBurst(bool requestHeat, bool sensorIsOk, float nowTempC, float activeSet, uint32_t nowMs){
  if(!requestHeat || !sensorIsOk) return false;

  float err = activeSet - nowTempC;
  if(err <= 0.0f) return false;
...
static void drawTabs(){
  ...
  drawGearIcon(tabSetR.x + tabSetR.w / 2, TAB_H / 2, 7, TFT_WHITE);
}
...
static void drawSettingRow(int idx, int y, const char* label, const String& value){
  const int rowH = 20;
  const int xLabel = PLOT_X + 10;
  const int xValue = PLOT_X + 118;
...
static void handleClick(int x, int y){
  // Tabs: only actual tab rectangles
  ...
  // Settings: one simple NEXT button
  if(mode == MODE_SETTINGS){
...
void loop(){
  ...
  if(pressed){
    ...
    if(touchDown){
      ...
      if(held >= 15 && held <= 900 && maxDx <= 45 && maxDy <= 45){
